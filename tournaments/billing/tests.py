...
class EntitlementResolverTests(TestCase):
    ...
    def test_effective_tier_comes_from_the_selected_payment_with_a_deterministic_tie(self):
        ...
        self.assertEqual(entitlement_for(user)['tier'], 'VIP')
        self.client.force_login(user)
        response = self.client.get(reverse('billing-status'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['subscription']['tier'], 'VIP')
        self.assertEqual(response.json()['entitlements']['tier'], 'VIP')
        self.assertEqual(response.json()['entitlements']['valid_until'], (now + timedelta(days=1)).isoformat())
        self.assertEqual(response.json()['entitlements']['capabilities']['pr'], 'full')
